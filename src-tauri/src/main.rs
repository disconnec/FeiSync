...
impl Default for TenantPermission {
    fn default() -> Self {
...
async fn dispatch_api_command(
    ...
    match result {
        Ok(value) => {
            let response_preview = summarize_value(&value);
            ...
            let state = app.state::<AppState>();
            let _ = state.inner().append_api_log(entry);
            Ok(value)
        }
        Err(err) => {
            let entry = ApiLogEntry {
...
fn sanitize_payload(payload: &Option<Value>) -> Option<Value> {
    payload.as_ref().map(|value| sanitize_value(value.clone()))
}
        ...
        Value::Object(map) => {
            ...
            for (key, val) in map.into_iter() {
                if is_sensitive_key(&key) {
                    sanitized.insert(key, Value::String("***".into()));
                } else {
                    sanitized.insert(key, sanitize_value(val));
                }
            }
...
fn summarize_value(value: &Value) -> Value {
    match value {
        ...
        _ => {
            let text = serde_json::to_string(value).unwrap_or_else(|_| String::from("<invalid>"));
            Value::String(truncate_string(&text, 800))
        }
    }
}
