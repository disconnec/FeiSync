...
const pickAutoTenant = (): { tenant: TenantInstance | null; reason: null | 'permission' | 'capacity' } => {
  const ordered = [...availableTenants.value].sort((a, b) => a.order - b.order);
  const EPSILON = 0.001;
  const selectByCapacity = (candidates: TenantInstance[]) =>
    candidates.find((tenant) => tenant.quota_gb - tenant.used_gb > EPSILON) ?? null;
  const writable = ordered.filter((tenant) => tenant.permission !== 'read_only');
  const writableCandidate = selectByCapacity(writable);
  if (writableCandidate) {
    return { tenant: writableCandidate, reason: null as null | 'permission' | 'capacity' };
  }
  if (writable.length) {
    return { tenant: null, reason: 'capacity' as const };
  }
  return { tenant: null, reason: 'permission' as const };
};
...
const ensureWritableParentToken = async (initialToken?: string | null) => {
  ...
  if (!parentToken) {
    const { tenant, reason } = pickAutoTenant();
    if (!tenant) {
      const message =
        reason === 'permission'
          ? '当前分组没有可用于写入的企业实例，请在设置中调整权限。'
          : '所有读写企业实例空间均已用尽，请扩容或启用更多实例后再试。';
      showAlert(message);
      addToast(message);
