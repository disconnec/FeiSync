    ...
    <form class="dialog" @submit.prevent="save">
      ...
      <label>
        ...
        <select v-model="form.permission">
          <option value="read_write">读写（可上传/下载）</option>
          <option value="read_only">只读（仅下载）</option>
        </select>
        <small>设置为只读后，所有上传、删除、重命名等写操作将被阻止。</small>
      </label>
...
import type { TenantInstance, TenantPermission } from '@/stores/tenantStore';
...
const form = reactive({
  ...
  permission: 'read_write' as TenantPermission
});
...
watch(
  ...
  async (tenant) => {
    if (tenant) {
      ...
      form.permission = detail.permission || 'read_write';
    } else {
      resetForm();
    }
  },
